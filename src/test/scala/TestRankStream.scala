...
class TestRankStream extends FunSuite {
  ...
  test(s"Check init for even") {
    ...
    assert(even1(0) == (3, 1.0))
    assert(even1(1) == (1, 2.0))
    assert(even1(2) == (4, 3.0))
    assert(even1(3) == (0, 4.0))
    assert(even1(4) == (5, 5.0))
    assert(even1(5) == (2, 6.0))
  }
  ...
  test("Test Scenario 2") {
    ...
    odd2.insert(10.0)
    odd2.insert(-1.0)
    assert(odd2(0) == (10, -1.0))
    assert(odd2(5) == (9, 10.0))
