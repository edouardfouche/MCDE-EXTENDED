...
class TestPreprocess extends FunSuite with TestData {
  test("MW Preprocessing works as expected, test case 1: Duplicates 'in the middle'") {
    ...
    assert(c(0)(1).adjustedrank === 1.5)
    assert(c(0)(2).adjustedrank === 1.5)
    assert(c(0)(6).adjustedrank === 7.0)
    assert(c(0)(7).adjustedrank === 7.0)
    assert(c(0)(8).adjustedrank === 7.0)
  }
  ...
  test("MW Preprocessing works as expected, test case 2. Duplicates at start and end") {
    ...
    val c = MWP().preprocess(new DataSet(Array(a.map(_.toDouble))))
    assert(c(0)(0).adjustedrank === 1.0)
    assert(c(0)(1).adjustedrank === 1.0)
    assert(c(0)(2).adjustedrank === 1.0)
    assert(c(0)(10).adjustedrank === 11.0)
    assert(c(0)(11).adjustedrank === 11.0)
    assert(c(0)(12).adjustedrank === 11.0)
  }
  ...
  test("MW Preprocessing works as expected, test case 3: No Duplicates") {
    ...
    val c = MWP().preprocess(new DataSet(Array(a.map(_.toDouble))))
    assert(c(0)(0).adjustedrank === 0.0)
    assert(c(0)(1).adjustedrank === 1.0)
    assert(c(0)(2).adjustedrank === 2.0)
    assert(c(0)(3).adjustedrank === 3.0)
    assert(c(0)(4).adjustedrank === 4.0)
    assert(c(0)(5).adjustedrank === 5.0)
    assert(c(0)(6).adjustedrank === 6.0)
    assert(c(0)(7).adjustedrank === 7.0)
    assert(c(0)(8).adjustedrank === 8.0)
    assert(c(0)(9).adjustedrank === 9.0)
  }
}
